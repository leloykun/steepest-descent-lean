...
def fixedBatchReducedLeadingProxy
  ...
  (S.fixedBatchLeadDriftConst * batchSize / (S.lambda * N * (1 - β)))
      * (1 + Real.log
          (S.initialExpectedSuboptimality * S.lambda * N * (1 - β)
...
def IsSmallBranchInteriorMomentumMinimizerFamilyFixedBatchReducedLeadingProxy
  ...
  ∃ cLogLower cLogUpper N0,
    ...
    ∀ N ≥ N0,
      ...
      S.fixedBatchLeadDriftConst * batchSize
        < S.initialExpectedSuboptimality * S.lambda * N * (1 - betaStar N) ∧
...
private theorem hasDerivAt_fixedBatchLeadingProxy
  ...
  have hNoise :
      HasDerivAt
        (fun _ : ℝ => S.fixedBatchLeadNoiseConst * Real.sqrt ((1 - β) / batchSize))
        0 η := by
    simpa using hasDerivAt_const η (S.fixedBatchLeadNoiseConst * Real.sqrt ((1 - β) / batchSize))
  simpa [fixedBatchLeadingProxy, add_assoc] using (hMain.add hNoise).add hDrift
...
private theorem fixedBatchLeading_interior_of_isEtaMinimizer
    ...
    S.fixedBatchLeadDriftConst * batchSize
      < S.initialExpectedSuboptimality * S.lambda * N * (1 - β) := by
  have hIsMinOn :
  ...
  have hLt' :
      S.fixedBatchLeadDriftConst
        < (S.initialExpectedSuboptimality * (S.lambda * N / batchSize)) * (1 - β) := by
    exact (div_lt_iff₀ hDeltaPos).1 hLt
  have hMul :
      S.fixedBatchLeadDriftConst * batchSize
        < ((S.initialExpectedSuboptimality * (S.lambda * N / batchSize)) * (1 - β)) * batchSize := by
    exact mul_lt_mul_of_pos_right hLt' hBatch
  calc
    S.fixedBatchLeadDriftConst * batchSize
      < ((S.initialExpectedSuboptimality * (S.lambda * N / batchSize)) * (1 - β)) * batchSize := hMul
    _ = S.initialExpectedSuboptimality * S.lambda * N * (1 - β) := by
          field_simp [hBatch.ne']
...
private theorem hasDerivAt_fixedBatchReducedLeadingProxy
  ...
  let K : ℝ := S.initialExpectedSuboptimality * S.lambda * N / (C * batchSize)
