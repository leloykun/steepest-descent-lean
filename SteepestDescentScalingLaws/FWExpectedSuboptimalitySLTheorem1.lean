...
/-!
Frank-Wolfe expected-suboptimality scaling laws, fixed-momentum family.

Upstream: `SteepestDescentScalingLaws.Commons` and the FW expected-suboptimality
bounds in `SteepestDescentOptimizationBounds`.
Downstream: the README summaries and the fixed-momentum FW expected-
suboptimality section of the blueprint.
-/
...
/-- Fixed-step large-horizon proxy for the FW expected-suboptimality bound. -/
def fWExpectedSuboptimalityProxySL1
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (η batchSize T β : ℝ) : ℝ :=
  S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * Real.exp (-(S.muFW * S.lambda * η * T))
    + S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize
    + S.fWExpectedSuboptimalityProxyDriftCoeff β * η

/-- Token-budget large-horizon proxy for the FW expected-suboptimality bound. -/
def fWExpectedSuboptimalityProxySL1Token
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (η batchSize N β : ℝ) : ℝ :=
...
def IsFixedStepFWExpectedSuboptimalityProxyMinimizer
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (η batchSize T β : ℝ) : Prop :=
  0 < η ∧
    ∀ η' > 0, S.fWExpectedSuboptimalityProxySL1 η batchSize T β ≤ S.fWExpectedSuboptimalityProxySL1 η' batchSize T β

/-- `etaStepStar` selects fixed-step SL1 minimizers. -/
def IsFixedStepFWExpectedSuboptimalityProxyMinimizerFamily
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (β batchSize : ℝ) (etaStepStar : ℝ → ℝ) : Prop :=
  ∀ {T : ℝ}, 0 < T →
    S.IsFixedStepFWExpectedSuboptimalityProxyMinimizer (etaStepStar T) batchSize T β

/-- `η` minimizes the token-budget SL1 proxy over positive learning rates. -/
def IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizer
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (η batchSize N β : ℝ) : Prop :=
  0 < η ∧
    ∀ η' > 0, S.fWExpectedSuboptimalityProxySL1Token η batchSize N β ≤ S.fWExpectedSuboptimalityProxySL1Token η' batchSize N β

/-- `etaTokenStar` selects token-budget SL1 minimizers. -/
def IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizerFamily
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (β : ℝ) (etaTokenStar : ℝ → ℝ → ℝ) : Prop :=
  ∀ {N batchSize : ℝ}, 0 < N → 0 < batchSize →
    S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizer (etaTokenStar N batchSize) batchSize N β

/-- Reduced token-budget proxy after substituting the closed-form interior optimizer. -/
def fixedMomentumFWExpectedSuboptimalityReducedProxy
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (N β batchSize : ℝ) : ℝ :=
  (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize / (S.muFW * S.lambda * N))
      * (1 + Real.log
          (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize)))
    + S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize

/-- `batchSize` minimizes the reduced token-budget SL1 proxy on the interior branch. -/
def IsFixedMomentumFWExpectedSuboptimalityReducedProxyBatchMinimizer
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (N β batchSize : ℝ) : Prop :=
  0 < batchSize ∧
    1 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize) ∧
    ∀ batchSize' > 0,
      1 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize') →
      S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β batchSize
        ≤ S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β batchSize'

/-- An eventually interior minimizer family on the small fixed-momentum token-budget branch. -/
def IsSmallBranchInteriorBatchMinimizerFamilyFixedMomentumFWExpectedSuboptimalityReducedProxy
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (β : ℝ) (batchSizeStar : ℝ → ℝ) : Prop :=
  ∃ cLogLower cLogUpper N0,
    0 < cLogLower ∧ 0 < cLogUpper ∧ 0 < N0 ∧
    ∀ N ≥ N0,
      S.IsFixedMomentumFWExpectedSuboptimalityReducedProxyBatchMinimizer N β (batchSizeStar N) ∧
      cLogLower * Real.log N
        ≤ Real.log
            (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N
              / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSizeStar N)) ∧
      Real.log
          (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N
            / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSizeStar N))
...
private def etaStarFixedStepsClosedForm
    ...
    (T β : ℝ) : ℝ :=
  (1 / (S.muFW * S.lambda * T))
    * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / S.fWExpectedSuboptimalityProxyDriftCoeff β)
...
private def fixedMomentumCriticalExpression
  ...
  batchSize ^ (3 / 2 : ℝ)
      * Real.log
          (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize))
    - (S.fWExpectedSuboptimalityProxyNoiseCoeff β * (S.muFW * S.lambda) / (2 * S.fWExpectedSuboptimalityProxyDriftCoeff β)) * N
...
private def IsInteriorCriticalPointFixedMomentumReducedProxy
  ...
  0 < batchSize ∧
    ...
    HasDerivAt (fun b => S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β b) 0 batchSize
...
private def etaScaleFixedSteps
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (T : ℝ) : ℝ :=
  Real.log T / (S.muFW * S.lambda * T)
private def etaScaleFixedMomentum
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (N : ℝ) : ℝ :=
  Real.rpow (Real.log N) (1 / 3 : ℝ) / (S.muFW * S.lambda * Real.rpow N (1 / 3 : ℝ))

/-! ------------------------------------------------------------------------
Private Lemmas and Theorems
------------------------------------------------------------------------ -/

private theorem fWExpectedSuboptimalityProxyDriftCoeff_pos
    (S : StochasticFrankWolfeKLGeometryContext Ω V) {β : ℝ}
    (hβ0 : 0 ≤ β) (hβ1 : β < 1) :
    0 < S.fWExpectedSuboptimalityProxyDriftCoeff β := by
  have hOneSub : 0 < 1 - β := sub_pos.mpr hβ1
  have hCoeff : 0 < 2 * S.L / (S.muFW * S.lambda) := by
    have : 0 < 2 * S.L := by
      exact mul_pos (by norm_num) S.L_pos
    exact div_pos this (mul_pos S.muFW_pos S.lambda_pos)
  have hTerm1 : 0 < (2 * S.L / (S.muFW * S.lambda)) * (1 + 2 * β ^ 2 / (1 - β)) := by
...
private theorem fWExpectedSuboptimalityProxyNoiseCoeff_nonneg
    (S : StochasticFrankWolfeKLGeometryContext Ω V) {β : ℝ}
    (hβ0 : 0 ≤ β) (hβ1 : β < 1) :
    0 ≤ S.fWExpectedSuboptimalityProxyNoiseCoeff β := by
  ...
  have hMid : 0 ≤ β * Real.sqrt ((1 - β) / (1 + β)) + (1 - β) := by
    ...
    linarith
  have hLeft : 0 ≤ 2 / (S.muFW * S.lambda) := by
    exact div_nonneg (by norm_num) (mul_pos S.muFW_pos S.lambda_pos).le
...
private theorem fixedMomentumFWExpectedSuboptimalityReducedProxy_eq
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    (N β batchSize : ℝ) :
    S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β batchSize
      = (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize / (S.muFW * S.lambda * N))
...
private theorem hasDerivAt_fixedMomentumFWExpectedSuboptimalityReducedProxy
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {β N batchSize : ℝ}
    (hβ0 : 0 ≤ β) (hβ1 : β < 1)
    ...
    HasDerivAt (fun b => S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β b)
      ((S.fWExpectedSuboptimalityProxyDriftCoeff β / (S.muFW * S.lambda * N)) * Real.log (S.fixedMomentumLogArg N β batchSize)
        - S.fWExpectedSuboptimalityProxyNoiseCoeff β / (2 * batchSize ^ (3 / 2 : ℝ))) batchSize := by
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  let B := S.fWExpectedSuboptimalityProxyNoiseCoeff β
  let C := S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / A
  have hApos : 0 < A := by
    dsimp [A]
    exact S.fWExpectedSuboptimalityProxyDriftCoeff_pos hβ0 hβ1
  have hCpos : 0 < C := by
    dsimp [C]
    ...
    exact div_pos (by simpa [mul_assoc] using hNum) hApos
  ...
  have hMainDeriv :
      HasDerivAt (fun b => (A / (S.muFW * S.lambda * N)) * (b * (1 + Real.log (C / b))))
        ((A / (S.muFW * S.lambda * N)) * Real.log (C / batchSize)) batchSize := by
    have hInner :
    ...
    simpa [mul_assoc, mul_left_comm, mul_comm] using hInner.const_mul (A / (S.muFW * S.lambda * N))
  have hInvSqrt :
  ...
  · funext b
    unfold fixedMomentumFWExpectedSuboptimalityReducedProxy C A B
    dsimp
    field_simp [hApos.ne']
  · rw [hLogEq]
...
private theorem isInteriorCriticalPointFixedMomentumReducedProxy_of_isBatchMinimizer
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hN : 0 < N)
    (hMin : S.IsFixedMomentumFWExpectedSuboptimalityReducedProxyBatchMinimizer N β batchSize) :
    S.IsInteriorCriticalPointFixedMomentumReducedProxy N β batchSize := by
  rcases hMin with ⟨hBatch, hInterior, hMinOn⟩
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  let C : ℝ := S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / A
  have hApos : 0 < A := by
    dsimp [A]
    exact S.fWExpectedSuboptimalityProxyDriftCoeff_pos hβ0 hβ1
  have hInteriorMul : A * batchSize < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N := by
  ...
  have hIsMinOn :
      IsMinOn (fun b => S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β b) (Set.Ioo 0 C) batchSize := by
    intro b hb
    have hbInterior : 1 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (A * b) := by
      have hMul : A * b < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N := by
        calc
          A * b < A * C := mul_lt_mul_of_pos_left hb.2 hApos
          _ = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N := by
                dsimp [C]
                field_simp [hApos.ne']
  ...
  have hLocalMin :
      IsLocalMin (fun b => S.fixedMomentumFWExpectedSuboptimalityReducedProxy N β b) batchSize := by
    exact hIsMinOn.localize.isLocalMin (Ioo_mem_nhds hBatch hBatchLtC)
  have hDeriv :=
    S.hasDerivAt_fixedMomentumFWExpectedSuboptimalityReducedProxy hβ0 hβ1 hGap hN hBatch
  have hDerivZero :
      ((S.fWExpectedSuboptimalityProxyDriftCoeff β / (S.muFW * S.lambda * N)) * Real.log (S.fixedMomentumLogArg N β batchSize)
        - S.fWExpectedSuboptimalityProxyNoiseCoeff β / (2 * batchSize ^ (3 / 2 : ℝ))) = 0 := by
...
private theorem fixedMomentumCriticalExpression_eq_zero_of_isInteriorCriticalPoint
  ...
  have hDerivFormula := S.hasDerivAt_fixedMomentumFWExpectedSuboptimalityReducedProxy hβ0 hβ1 hGap hN hBatch
  have hEqDeriv :
      ((S.fWExpectedSuboptimalityProxyDriftCoeff β / (S.muFW * S.lambda * N)) * Real.log (S.fixedMomentumLogArg N β batchSize)
        - S.fWExpectedSuboptimalityProxyNoiseCoeff β / (2 * batchSize ^ (3 / 2 : ℝ))) = 0 := by
    exact hDerivFormula.unique hDerivZero
  have hApos : 0 < S.fWExpectedSuboptimalityProxyDriftCoeff β := S.fWExpectedSuboptimalityProxyDriftCoeff_pos hβ0 hβ1
  have hMul :
      batchSize ^ (3 / 2 : ℝ)
        * ((S.fWExpectedSuboptimalityProxyDriftCoeff β / (S.muFW * S.lambda * N)) * Real.log (S.fixedMomentumLogArg N β batchSize)
            - S.fWExpectedSuboptimalityProxyNoiseCoeff β / (2 * batchSize ^ (3 / 2 : ℝ))) = 0 := by
  ...
  have hExpanded :
      S.fixedMomentumCriticalExpression N β batchSize
        = (S.muFW * S.lambda * N / S.fWExpectedSuboptimalityProxyDriftCoeff β)
            * (batchSize ^ (3 / 2 : ℝ)
                * ((S.fWExpectedSuboptimalityProxyDriftCoeff β / (S.muFW * S.lambda * N))
                    * Real.log (S.fixedMomentumLogArg N β batchSize)
                  - S.fWExpectedSuboptimalityProxyNoiseCoeff β / (2 * batchSize ^ (3 / 2 : ℝ)))) := by
    unfold fixedMomentumCriticalExpression fixedMomentumLogArg
    field_simp [S.muFW_pos.ne', S.lambda_pos.ne', hN.ne', hApos.ne', hBatchPowNe]
...
private theorem etaScaleFixedMomentum_eq
    (S : StochasticFrankWolfeKLGeometryContext Ω V) {N : ℝ}
    (hN : 0 < N) (hlog : 0 < Real.log N) :
    etaScaleFixedMomentum S N
      = (1 / (S.muFW * S.lambda)) * Real.rpow (Real.log N / N) (1 / 3 : ℝ) := by
  unfold etaScaleFixedMomentum
  calc
    Real.rpow (Real.log N) (1 / 3 : ℝ) / (S.muFW * S.lambda * Real.rpow N (1 / 3 : ℝ))
      = (Real.rpow (Real.log N) (1 / 3 : ℝ) / Real.rpow N (1 / 3 : ℝ)) / (S.muFW * S.lambda) := by
          field_simp [S.muFW_pos.ne', S.lambda_pos.ne']
    _ = Real.rpow (Real.log N / N) (1 / 3 : ℝ) / (S.muFW * S.lambda) := by
          congr 1
          simpa [div_eq_mul_inv] using (Real.div_rpow hlog.le hN.le (1 / 3 : ℝ)).symm
    _ = (1 / (S.muFW * S.lambda)) * Real.rpow (Real.log N / N) (1 / 3 : ℝ) := by ring
...
private theorem hasDerivAt_fWExpectedSuboptimalityProxySL1
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {η batchSize T β : ℝ} :
    HasDerivAt (fun η' => S.fWExpectedSuboptimalityProxySL1 η' batchSize T β)
      (-(S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T) * Real.exp (-(S.muFW * S.lambda * η * T)))
        + S.fWExpectedSuboptimalityProxyDriftCoeff β) η := by
  have hInner :
      HasDerivAt (fun η' : ℝ => -(S.muFW * S.lambda * η' * T)) (-(S.muFW * S.lambda * T)) η := by
    have h :=
      ((hasDerivAt_id η).const_mul (S.muFW * S.lambda * T)).neg
    simpa [mul_assoc, mul_left_comm, mul_comm] using h
  have hExp := (Real.hasDerivAt_exp (-(S.muFW * S.lambda * η * T))).comp η hInner
  have hMain :
      HasDerivAt
        (fun η' : ℝ => S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * Real.exp (-(S.muFW * S.lambda * η' * T)))
        (-(S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T) * Real.exp (-(S.muFW * S.lambda * η * T)))) η := by
...
private theorem hasDerivAt_fWExpectedSuboptimalityProxySL1Token
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {η batchSize N β : ℝ} :
    HasDerivAt (fun η' => S.fWExpectedSuboptimalityProxySL1Token η' batchSize N β)
...
private theorem closedForm_fixedStep_isMinimizer
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hT : 0 < T)
    (hInterior : S.fWExpectedSuboptimalityProxyDriftCoeff β < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T) :
    S.IsFixedStepFWExpectedSuboptimalityProxyMinimizer (S.etaStarFixedStepsClosedForm T β) batchSize T β := by
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  let a : ℝ := S.muFW * S.lambda * T
  ...
  have haPos : 0 < a := by
    dsimp [a]
    exact mul_pos (mul_pos S.muFW_pos S.lambda_pos) hT
  have hArgPos :
      0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A := by
    have hNum : 0 < (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda) * T := by
      exact mul_pos (mul_pos (mul_pos hGap S.muFW_pos) S.lambda_pos) hT
    exact div_pos (by simpa [mul_assoc] using hNum) hApos
  have hArgGtOne : 1 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A := by
  ...
  have hEtaPos : 0 < ηStar := by
    ...
    exact one_div_pos.mpr (mul_pos (mul_pos S.muFW_pos S.lambda_pos) hT)
  have hExpStar :
    ...
    have hMul : a * ηStar = Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) := by
      dsimp [a, ηStar, etaStarFixedStepsClosedForm]
      field_simp [S.muFW_pos.ne', S.lambda_pos.ne', hT.ne']
      ring
    calc
      S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * Real.exp (-(a * ηStar))
          = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality
              * Real.exp (-Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A)) := by
                rw [hMul]
      _ = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality / (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) := by
            rw [Real.exp_neg, Real.exp_log hArgPos]
            simp [div_eq_mul_inv]
      _ = A / a := by
            dsimp [a]
            field_simp [hApos.ne', (mul_pos S.muFW_pos S.lambda_pos).ne', hT.ne', hGap.ne']
  refine ⟨hEtaPos, ?_⟩
  intro η hη
  let u : ℝ := a * (η - ηStar)
  ...
  calc
    S.fWExpectedSuboptimalityProxySL1 ηStar batchSize T β
      = S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize + A / a + A * ηStar := by
          unfold fWExpectedSuboptimalityProxySL1
          have hExpArg : -(S.muFW * S.lambda * ηStar * T) = -(a * ηStar) := by
            dsimp [a]
            ring
          rw [hExpArg]
          rw [hExpStar]
          ring
    _ ≤ S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize + ((A / a) * Real.exp (-u) + A * (η - ηStar)) + A * ηStar := by
          gcongr
    _ = S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize + (A / a) * Real.exp (-u) + A * η := by ring
    _ = S.fWExpectedSuboptimalityProxySL1 η batchSize T β := by
          unfold fWExpectedSuboptimalityProxySL1
          have hExpArg : -(S.muFW * S.lambda * η * T) = -(a * η) := by
            dsimp [a]
            ring
          rw [hExpArg]
          rw [hExpEta]
...
private theorem fixedToken_interior_of_isMinimizer
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hN : 0 < N) (hBatch : 0 < batchSize)
    (hMin : S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizer ηStar batchSize N β) :
    S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N := by
  have hIsMinOn : IsMinOn (fun η => S.fWExpectedSuboptimalityProxySL1Token η batchSize N β) (Set.Ioi 0) ηStar := by
    intro η hη
    exact hMin.2 η hη
  have hLocalMin : IsLocalMin (fun η => S.fWExpectedSuboptimalityProxySL1Token η batchSize N β) ηStar := by
    exact hIsMinOn.localize.isLocalMin (Ioi_mem_nhds hMin.1)
  have hDerivZero :
      -(S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * N / batchSize)
          * Real.exp (-(S.muFW * S.lambda * ηStar * N / batchSize)))
        + S.fWExpectedSuboptimalityProxyDriftCoeff β = 0 := by
    exact hLocalMin.hasDerivAt_eq_zero (S.hasDerivAt_fWExpectedSuboptimalityProxySL1Token (η := ηStar))
  ...
  have hLt :
      S.fWExpectedSuboptimalityProxyDriftCoeff β
  ...
  have hMul :
      S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize
        < (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * N / batchSize)) * batchSize := by
    exact mul_lt_mul_of_pos_right hLt hBatch
  calc
    S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize
      < (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * N / batchSize)) * batchSize := hMul
    _ = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N := by
          field_simp [hBatch.ne']
...
private theorem closedForm_fixedStep_lt_of_ne
    ...
    S.fWExpectedSuboptimalityProxySL1 (S.etaStarFixedStepsClosedForm T β) batchSize T β
      < S.fWExpectedSuboptimalityProxySL1 η batchSize T β := by
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  let a : ℝ := S.muFW * S.lambda * T
  let ηStar := S.etaStarFixedStepsClosedForm T β
  let u : ℝ := a * (η - ηStar)
  ...
  have huNe : u ≠ 0 := by
    ...
    · exact (ne_of_gt (mul_pos (mul_pos S.muFW_pos S.lambda_pos) hT))
...
private theorem closedForm_fixedToken_isMinimizer
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hN : 0 < N) (hBatch : 0 < batchSize)
    (hInterior : S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N) :
    S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizer (S.etaStarFixedMomentumClosedForm N β batchSize)
      batchSize N β := by
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  ...
  have hArgPos :
      0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (A * batchSize) := by
    have hNum : 0 < (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda) * N := by
      exact mul_pos (mul_pos (mul_pos hGap S.muFW_pos) S.lambda_pos) hN
    exact div_pos (by simpa [mul_assoc] using hNum) (mul_pos hApos hBatch)
  have hArgGtOne : 1 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (A * batchSize) := by
    have hDenPos : 0 < A * batchSize := mul_pos hApos hBatch
  ...
  have hExpStar :
      S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * Real.exp (-(a * ηStar)) = A / a := by
    have hMul : a * ηStar = Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (A * batchSize)) := by
    ...
    calc
      S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * Real.exp (-(a * ηStar))
          = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality
              * Real.exp (-Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (A * batchSize))) := by
                rw [hMul]
      _ = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality / (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (A * batchSize)) := by
            rw [Real.exp_neg, Real.exp_log hArgPos]
            simp [div_eq_mul_inv]
      _ = A / a := by
            dsimp [a]
            field_simp [hApos.ne', (mul_pos S.muFW_pos S.lambda_pos).ne', hN.ne', hBatch.ne', hGap.ne']
  refine ⟨hEtaPos, ?_⟩
  intro η hη
  let u : ℝ := a * (η - ηStar)
  have hOneLe : 1 ≤ Real.exp (-u) + u := by
  ...
  have hExpEta :
    ...
    calc
      ...
      _ = (A / a) * Real.exp (-u) := by rw [hExpStar]
  calc
    S.fWExpectedSuboptimalityProxySL1Token ηStar batchSize N β
      = S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize + A / a + A * ηStar := by
          unfold fWExpectedSuboptimalityProxySL1Token
    ...
    _ ≤ S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize + ((A / a) * Real.exp (-u) + A * (η - ηStar)) + A * ηStar := by
          gcongr
    _ = S.fWExpectedSuboptimalityProxyNoiseCoeff β / Real.sqrt batchSize + (A / a) * Real.exp (-u) + A * η := by ring
    _ = S.fWExpectedSuboptimalityProxySL1Token η batchSize N β := by
          unfold fWExpectedSuboptimalityProxySL1Token
...
private theorem closedForm_fixedToken_lt_of_ne
    ...
    S.fWExpectedSuboptimalityProxySL1Token (S.etaStarFixedMomentumClosedForm N β batchSize) batchSize N β
      < S.fWExpectedSuboptimalityProxySL1Token η batchSize N β := by
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  let a : ℝ := S.muFW * S.lambda * N / batchSize
  let ηStar := S.etaStarFixedMomentumClosedForm N β batchSize
  let u : ℝ := a * (η - ηStar)
...
private theorem fixedStep_interior_of_isMinimizer
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hT : 0 < T)
    (hMin : S.IsFixedStepFWExpectedSuboptimalityProxyMinimizer ηStar batchSize T β) :
    S.fWExpectedSuboptimalityProxyDriftCoeff β < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T := by
  have hIsMinOn : IsMinOn (fun η => S.fWExpectedSuboptimalityProxySL1 η batchSize T β) (Set.Ioi 0) ηStar := by
    intro η hη
    exact hMin.2 η hη
  have hLocalMin : IsLocalMin (fun η => S.fWExpectedSuboptimalityProxySL1 η batchSize T β) ηStar := by
    exact hIsMinOn.localize.isLocalMin (Ioi_mem_nhds hMin.1)
  have hDerivZero :
      -(S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T) * Real.exp (-(S.muFW * S.lambda * ηStar * T)))
        + S.fWExpectedSuboptimalityProxyDriftCoeff β = 0 := by
    exact hLocalMin.hasDerivAt_eq_zero (S.hasDerivAt_fWExpectedSuboptimalityProxySL1 (η := ηStar))
  have hExpLtOne : Real.exp (-(S.muFW * S.lambda * ηStar * T)) < 1 := by
    apply Real.exp_lt_one_iff.mpr
    have hArgPos : 0 < S.muFW * S.lambda * ηStar * T := by
      exact mul_pos (mul_pos (mul_pos S.muFW_pos S.lambda_pos) hMin.1) hT
    linarith
  have hScalePos : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T) := by
    exact mul_pos hGap (mul_pos (mul_pos S.muFW_pos S.lambda_pos) hT)
  calc
    S.fWExpectedSuboptimalityProxyDriftCoeff β
      = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T) * Real.exp (-(S.muFW * S.lambda * ηStar * T)) := by
          linarith
    _ < (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T)) * 1 := by
          exact mul_lt_mul_of_pos_left hExpLtOne hScalePos
    _ = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * (S.muFW * S.lambda * T) := by ring
    _ = S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T := by ring
...
private theorem etaStarFixedSteps_eq
    ...
    ηStar
      = (1 / (S.muFW * S.lambda * T))
          * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / S.fWExpectedSuboptimalityProxyDriftCoeff β) := by
...
private theorem fixedStepClosedFormFamily_eq
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    {etaStepStar : ℝ → ℝ}
    (hMin : S.IsFixedStepFWExpectedSuboptimalityProxyMinimizerFamily β batchSize etaStepStar) :
    ∀ {T : ℝ}, 0 < T →
      etaStepStar T
        = (1 / (S.muFW * S.lambda * T))
            * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / S.fWExpectedSuboptimalityProxyDriftCoeff β) := by
...
private theorem fixedStepIterationScalingBounds
    ...
    ∃ cLower cUpper T0,
      ...
      ∀ T ≥ T0,
        cLower * (Real.log T / (S.muFW * S.lambda * T)) ≤ etaStepStar T ∧
        etaStepStar T ≤ cUpper * (Real.log T / (S.muFW * S.lambda * T)) := by
  let A := S.fWExpectedSuboptimalityProxyDriftCoeff β
  let c : ℝ := Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda / A)
  let T0 : ℝ := max (Real.exp 1) (max (Real.exp (2 * |c|)) (A / (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda) + 1))
  have hApos : 0 < A := by
    dsimp [A]
    exact S.fWExpectedSuboptimalityProxyDriftCoeff_pos hβ0 hβ1
  have hCpos : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda / A := by
    have hNum : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda := by
      exact mul_pos (mul_pos hGap S.muFW_pos) S.lambda_pos
  ...
  have hEqStar :
      etaStepStar T
        = (1 / (S.muFW * S.lambda * T))
            * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) := by
  ...
  have hAbsLe : |c| ≤ Real.log T / 2 := by
    have hExpBound : Real.exp (2 * |c|) ≤ T := by
      ...
      exact le_trans
        (le_trans
          (le_max_left (Real.exp (2 * |c|)) (A / (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda) + 1))
    ...
    linarith
  have hSplit :
      Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) = Real.log T + c := by
    unfold c
    have hRewrite :
        S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A
          = T * (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda / A) := by
      field_simp [hApos.ne']
    rw [hRewrite, Real.log_mul hTpos.ne' hCpos.ne']
  have hLowerLog : (Real.log T) / 2 ≤ Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) := by
    rw [hSplit]
    have : -|c| ≤ c := by exact neg_abs_le c
    linarith
  have hUpperLog : Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) ≤ 2 * Real.log T := by
    rw [hSplit]
    have : c ≤ |c| := le_abs_self c
    linarith
  rw [hEqStar]
  have hFactor : 0 ≤ 1 / (S.muFW * S.lambda * T) := by
    exact one_div_nonneg.mpr (mul_nonneg (mul_pos S.muFW_pos S.lambda_pos).le hTpos.le)
  have hScaled := scale_bounds_of_nonneg hFactor hLowerLog hUpperLog
  constructor
  · have hLowerScaled :
        (1 / (S.muFW * S.lambda * T)) * ((Real.log T) / 2)
          ≤ (1 / (S.muFW * S.lambda * T))
              * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A) := hScaled.1
    have hRewrite :
        (1 / 2 : ℝ) * (Real.log T / (S.muFW * S.lambda * T))
          = (1 / (S.muFW * S.lambda * T)) * ((Real.log T) / 2) := by
      field_simp [S.muFW_pos.ne', S.lambda_pos.ne', hTpos.ne']
    rw [hRewrite]
    simpa [A] using hLowerScaled
  · have hUpperScaled :
        (1 / (S.muFW * S.lambda * T))
            * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / A)
          ≤ (1 / (S.muFW * S.lambda * T)) * (2 * Real.log T) := hScaled.2
    have hRewrite :
        (1 / (S.muFW * S.lambda * T)) * (2 * Real.log T)
          = (2 : ℝ) * (Real.log T / (S.muFW * S.lambda * T)) := by
      field_simp [S.muFW_pos.ne', S.lambda_pos.ne', hTpos.ne']
    rw [← hRewrite]
...
private theorem etaStarFixedMomentum_eq
    ...
    ηStar
      = (batchSize / (S.muFW * S.lambda * N))
          * Real.log
              (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSize)) := by
...
private theorem fixedTokenClosedFormFamily_eq
    ...
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    {etaTokenStar : ℝ → ℝ → ℝ}
    (hMin : S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizerFamily β etaTokenStar) :
    ∀ {N batchSize : ℝ}, 0 < N → 0 < batchSize →
...
private theorem fixedMomentumTokenBudgetScalingBounds
    ...
    {β : ℝ} (hβ0 : 0 ≤ β) (hβ1 : β < 1)
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hNoise : 0 < S.fWExpectedSuboptimalityProxyNoiseCoeff β)
    {batchSizeStar : ℝ → ℝ}
    (hBatchMin :
      S.IsSmallBranchInteriorBatchMinimizerFamilyFixedMomentumFWExpectedSuboptimalityReducedProxy β batchSizeStar)
    {etaTokenStar : ℝ → ℝ → ℝ}
    (hMin : S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizerFamily β etaTokenStar) :
  ...
  have hKpos : 0 < K := by
    dsimp [K, B, A]
    have : 0 < B * (S.muFW * S.lambda) := mul_pos hNoise (mul_pos S.muFW_pos S.lambda_pos)
  ...
  · intro N hN
    ...
    have hEtaEqClosed :
        etaTokenStar N (batchSizeStar N)
          = (batchSizeStar N / (S.muFW * S.lambda * N))
              * Real.log
                  (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N
                    / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSizeStar N)) := by
      ...
      calc
        etaTokenStar N (batchSizeStar N)
            = S.etaStarFixedMomentumClosedForm N β (batchSizeStar N) := by
                trans ((batchSizeStar N / (S.muFW * S.lambda * N))
                    * Real.log
                        (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * N
                          / (S.fWExpectedSuboptimalityProxyDriftCoeff β * batchSizeStar N)))
                · exact hEtaEqClosed
    ...
    have hEtaLower' :
      ...
      calc
        cEtaLower * etaScaleFixedMomentum S N
            = (1 / (S.muFW * S.lambda)) * (cEtaLower * (iterationScale N * Real.log N / N)) := by
      ...
      calc
        etaTokenStar N (batchSizeStar N)
            = (1 / (S.muFW * S.lambda))
                * (batchSizeStar N * Real.log (S.fixedMomentumLogArg N β (batchSizeStar N)) / N) :=
              hEtaEq
        _ ≤ (1 / (S.muFW * S.lambda)) * (cEtaUpper * (iterationScale N * Real.log N / N)) :=
              hEtaScaleBounds.2
...
theorem fWExpectedSuboptimalitySLTheorem1_1_iterationScaling
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {β batchSize : ℝ} (hβ0 : 0 ≤ β) (hβ1 : β < 1)
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality) :
    ∀ {etaStepStar : ℝ → ℝ},
      S.IsFixedStepFWExpectedSuboptimalityProxyMinimizerFamily β batchSize etaStepStar →
      (∀ {T : ℝ}, 0 < T →
        etaStepStar T
          = (1 / (S.muFW * S.lambda * T))
              * Real.log (S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality * S.muFW * S.lambda * T / S.fWExpectedSuboptimalityProxyDriftCoeff β)) ∧
  ...
  refine ⟨cLowerOld / (S.muFW * S.lambda), cUpperOld / (S.muFW * S.lambda), T0,
    div_pos hcLowerOld (mul_pos S.muFW_pos S.lambda_pos), div_pos hcUpperOld (mul_pos S.muFW_pos S.lambda_pos), hT0, ?_⟩
  intro T hT
  have hTpos : 0 < T := lt_of_lt_of_le hT0 hT
  rcases hBounds T hT with ⟨hLowerOld, hUpperOld⟩
  constructor
  · calc
      (cLowerOld / (S.muFW * S.lambda)) * (Real.log T / T)
          = cLowerOld * (Real.log T / (S.muFW * S.lambda * T)) := by
              field_simp [S.muFW_pos.ne', S.lambda_pos.ne', hTpos.ne']
      _ ≤ etaStepStar T := hLowerOld
  · calc
      etaStepStar T ≤ cUpperOld * (Real.log T / (S.muFW * S.lambda * T)) := hUpperOld
      _ = (cUpperOld / (S.muFW * S.lambda)) * (Real.log T / T) := by
            field_simp [S.muFW_pos.ne', S.lambda_pos.ne', hTpos.ne']

theorem fWExpectedSuboptimalitySLTheorem1_2_tokenBudgetScaling
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {β : ℝ} (hβ0 : 0 ≤ β) (hβ1 : β < 1)
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hNoise : 0 < S.fWExpectedSuboptimalityProxyNoiseCoeff β)
    ...
    (hBatchMin :
      S.IsSmallBranchInteriorBatchMinimizerFamilyFixedMomentumFWExpectedSuboptimalityReducedProxy β batchSizeStar) :
    ∀ {etaTokenStar : ℝ → ℝ → ℝ},
      S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizerFamily β etaTokenStar →
      (∀ {N batchSize : ℝ}, 0 < N → 0 < batchSize →
        etaTokenStar N batchSize
          = (batchSize / (S.muFW * S.lambda * N))
              * Real.log
  ...
  refine ⟨cBatchLower, cBatchUpper, cEtaLowerOld / (S.muFW * S.lambda), cEtaUpperOld / (S.muFW * S.lambda), N0,
    hcBatchLower, hcBatchUpper, div_pos hcEtaLowerOld (mul_pos S.muFW_pos S.lambda_pos),
...
theorem fWExpectedSuboptimalitySLTheorem1_FixedMomentumLargeHorizonProxy
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {β batchSize : ℝ} (hβ0 : 0 ≤ β) (hβ1 : β < 1)
    (hGap : 0 < S.toStochasticFrankWolfeGeometryContext.toStochasticSteepestDescentGeometryContext.initialSuboptimality)
    (hNoise : 0 < S.fWExpectedSuboptimalityProxyNoiseCoeff β)
    {etaStepStar : ℝ → ℝ}
    (hStepMin : S.IsFixedStepFWExpectedSuboptimalityProxyMinimizerFamily β batchSize etaStepStar)
    {etaTokenStar : ℝ → ℝ → ℝ}
    (hTokenMin : S.IsFixedTokenBudgetFWExpectedSuboptimalityProxyMinimizerFamily β etaTokenStar)
    {batchSizeStar : ℝ → ℝ}
    (hBatchMin :
      S.IsSmallBranchInteriorBatchMinimizerFamilyFixedMomentumFWExpectedSuboptimalityReducedProxy β batchSizeStar) :
    ∃ cStepLower cStepUpper T0 cBatchLower cBatchUpper cEtaLower cEtaUpper N0,
      ...
      (∀ N ≥ N0,
        ...
        etaTokenStar N (batchSizeStar N) ≤
          cEtaUpper * (Real.rpow (Real.log N) (1 / 3 : ℝ)
            / Real.rpow N (1 / 3 : ℝ))) := by
  rcases S.fWExpectedSuboptimalitySLTheorem1_1_iterationScaling (batchSize := batchSize) hβ0 hβ1 hGap
      (etaStepStar := etaStepStar) hStepMin with
    ⟨_, ⟨cStepLower, cStepUpper, T0, hcStepLower, hcStepUpper, hT0, hStep⟩⟩
  rcases S.fWExpectedSuboptimalitySLTheorem1_2_tokenBudgetScaling hβ0 hβ1 hGap hNoise hBatchMin
