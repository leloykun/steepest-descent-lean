...
def fixedBatchFWExpectedSuboptimalityLeadDriftConst
  ...
  4 * S.L / (S.muFW * S.lambda) + 2 * S.initialGradNorm
...
def fixedBatchFWExpectedSuboptimalityReducedLeadingProxy
  ...
  (S.fixedBatchFWExpectedSuboptimalityLeadDriftConst * batchSize / (S.muFW * S.lambda * N * (1 - β)))
      * (1 + Real.log
          (S.frankWolfeExpectedSuboptimalityInitialGap * S.muFW * S.lambda * N * (1 - β)
...
def IsSmallBranchInteriorMomentumMinimizerFamilyFixedBatchFWExpectedSuboptimalityReducedLeadingProxy
  ...
  ∃ cLogLower cLogUpper N0,
    ...
    ∀ N ≥ N0,
      ...
      S.fixedBatchFWExpectedSuboptimalityLeadDriftConst * batchSize
        < S.frankWolfeExpectedSuboptimalityInitialGap * S.muFW * S.lambda * N * (1 - betaStar N) ∧
...
private theorem closedForm_fixedBatchLeading_isMinimizer
  ...
  have hArgPos : 0 < S.fixedBatchLeadingLogArg N β batchSize := by
    ...
    have hNum :
        0 < ((S.frankWolfeExpectedSuboptimalityInitialGap * S.muFW * S.lambda) * N) * (1 - β) := by
  ...
  have hExpStar :
    ...
    calc
      S.frankWolfeExpectedSuboptimalityInitialGap * Real.exp (-(a * ηStar))
          = S.frankWolfeExpectedSuboptimalityInitialGap * Real.exp (-Real.log (S.fixedBatchLeadingLogArg N β batchSize)) := by
              rw [hMul]
      _ = S.frankWolfeExpectedSuboptimalityInitialGap / S.fixedBatchLeadingLogArg N β batchSize := by
...
private theorem fixedBatchLeading_interior_of_isEtaMinimizer
    ...
    S.fixedBatchFWExpectedSuboptimalityLeadDriftConst * batchSize
      < S.frankWolfeExpectedSuboptimalityInitialGap * S.muFW * S.lambda * N * (1 - β) := by
  have hIsMinOn :
  ...
  have hLt' :
      S.fixedBatchFWExpectedSuboptimalityLeadDriftConst
        < (S.frankWolfeExpectedSuboptimalityInitialGap * (S.muFW * S.lambda * N / batchSize)) * (1 - β) := by
    exact (div_lt_iff₀ hDeltaPos).1 hLt
  have hMul :
      S.fixedBatchFWExpectedSuboptimalityLeadDriftConst * batchSize
        < ((S.frankWolfeExpectedSuboptimalityInitialGap * (S.muFW * S.lambda * N / batchSize)) * (1 - β)) * batchSize := by
    exact mul_lt_mul_of_pos_right hLt' hBatch
  calc
    S.fixedBatchFWExpectedSuboptimalityLeadDriftConst * batchSize
      < ((S.frankWolfeExpectedSuboptimalityInitialGap * (S.muFW * S.lambda * N / batchSize)) * (1 - β)) * batchSize := hMul
    _ = S.frankWolfeExpectedSuboptimalityInitialGap * S.muFW * S.lambda * N * (1 - β) := by
          field_simp [hBatch.ne']
...
private theorem fixedBatchLeadingClosedFormFamily_eq
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {batchSize : ℝ} (hGap : 0 < S.frankWolfeExpectedSuboptimalityInitialGap) (hBatch : 0 < batchSize)
...
private theorem hasDerivAt_fixedBatchFWExpectedSuboptimalityReducedLeadingProxy
  ...
  let K : ℝ := S.frankWolfeExpectedSuboptimalityInitialGap * S.muFW * S.lambda * N / (C * batchSize)
...
private theorem fixedBatchLeadingTokenBudgetScalingBounds
    (S : StochasticFrankWolfeKLGeometryContext Ω V)
    {batchSize : ℝ}
    (hGap : 0 < S.frankWolfeExpectedSuboptimalityInitialGap) (hBatch : 0 < batchSize)
    (hNoise : 0 < S.fixedBatchFWExpectedSuboptimalityLeadNoiseConst)
    {betaStar : ℝ → ℝ}
    (hMomentum :
