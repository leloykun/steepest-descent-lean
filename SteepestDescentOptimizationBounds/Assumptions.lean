...
/--
Sigma-algebra generated by the current iterate `W_t` and all samples strictly
preceding `(t, i)` in lexicographic order.
-/
...
def path (S : StochasticSteepestDescentGeometryContext Ω V) (ω : Ω) :
  ...
  W := fun t => S.W t ω
  W0_bound := by
    have hEq : S.W 0 ω = S.W0 := S.W_zero ω
    simpa [hEq] using S.W0_bound
  WStar := S.WStar
