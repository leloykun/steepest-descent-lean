...
/--
Sigma-algebra generated by the initial iterate `W_0`, the current iterate `W_t`,
and all samples strictly preceding `(t, i)` in lexicographic order.
-/
def samplePrefixSigma
  ...
  MeasurableSpace.comap (W 0) (inferInstance : MeasurableSpace V) ⊔
...
closed ball `‖x‖ ≤ radius`.
-/
structure Assumption3_FLocalSmoothness
    {V : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
    (fGrad : V → StrongDual ℝ V) (radius : ℝ) where
  L : ℝ
  local_lipschitz : LocalLipschitzOnClosedBallUnderNormPair fGrad radius L

namespace Assumption3_FLocalSmoothness

variable {V : Type*}
variable [NormedAddCommGroup V] [NormedSpace ℝ V]
variable {fGrad : V → StrongDual ℝ V} {radius : ℝ}

lemma pos (h : Assumption3_FLocalSmoothness fGrad radius) : 0 < h.L :=
  h.local_lipschitz.pos

lemma nonneg (h : Assumption3_FLocalSmoothness fGrad radius) : 0 ≤ h.L :=
  h.local_lipschitz.nonneg

lemma bound (h : Assumption3_FLocalSmoothness fGrad radius) :
...
structure SteepestDescentPathGeometryContext
  ...
  assumption3_fLocalSmoothness : Assumption3_FLocalSmoothness fGrad (1 / lambda)
...
structure AssumptionFrankWolfeKLAlongIterates
    ...
    (WStar : V)
    (W : ℕ → V)
    (muFW constraintRadius : ℝ) where
  gap_lower_bound :
    ∀ t,
      muFW * (f (W t) - f WStar) ≤
        sSup ((fun V => (fGrad (W t)) (W t - V)) '' Metric.closedBall (0 : V) constraintRadius)
...
structure FrankWolfeKLPathGeometryContext
  ...
  assumptionFrankWolfeKL :
    AssumptionFrankWolfeKLAlongIterates V f fGrad WStar W muFW (1 / lambda)
...
structure StochasticSteepestDescentGeometryContext
  ...
  fderiv_eq : ∀ X, ‖X‖ ≤ 1 / lambda → HasFDerivAt f (fGrad X) X
  W : ℕ → Ω → V
  W_zero_measurable : Measurable (W 0)
  W_zero_bound : ∀ ω, ‖W 0 ω‖ ≤ 1 / lambda
  WStar : V
  WStar_optimality : ∀ W, f WStar ≤ f W
  ...
  assumption3_fLocalSmoothness :
    Assumption3_FLocalSmoothness fGrad (1 / lambda)
  assumption4_localProxyPotential :
    Assumption4_LocalSmoothProxyPotential V
...
/-- The decoupled-weight-decay primal feasibility radius `1 / λ`. -/
def constraintRadius (S : SteepestDescentPathGeometryContext V) : ℝ :=
  1 / S.lambda

/-- The generic primal feasibility radius carried by the path geometry. -/
lemma constraintRadius_pos (S : SteepestDescentPathGeometryContext V) : 0 < S.constraintRadius := by
  simpa [SteepestDescentPathGeometryContext.constraintRadius] using
    (one_div_pos.mpr S.lambda_pos)

/-- The generic primal feasibility radius is nonnegative. -/
lemma constraintRadius_nonneg (S : SteepestDescentPathGeometryContext V) : 0 ≤ S.constraintRadius :=
  S.constraintRadius_pos.le

/-- The pathwise radius specializes to the decoupled weight-decay value `1 / λ`. -/
lemma lambda_mul_constraintRadius_eq_one (S : SteepestDescentPathGeometryContext V) :
    S.lambda * S.constraintRadius = 1 := by
...
/-- The shared primal feasibility ball carried by the path geometry. -/
def constraintBall (S : SteepestDescentPathGeometryContext V) : Set V :=
  Metric.closedBall 0 S.constraintRadius
...
/-- The pathwise suboptimality gap relative to the reference point `W_*`. -/
def suboptimality (S : StarConvexPathGeometryContext V) (t : ℕ) : ℝ :=
...
/-- The pathwise suboptimality gap relative to the shared reference point `W_*`. -/
def suboptimality (S : FrankWolfePathGeometryContext V) (t : ℕ) : ℝ :=
  S.f (S.W t) - S.f S.WStar

/-- The deterministic initial suboptimality `f(W_0) - f(W_*)`. -/
def initialSuboptimality (S : FrankWolfePathGeometryContext V) : ℝ :=
  S.suboptimality 0
...
/-- The decoupled-weight-decay primal feasibility radius `1 / λ`. -/
def constraintRadius (S : StochasticSteepestDescentGeometryContext Ω V) : ℝ :=
  1 / S.lambda

/-- The generic primal feasibility radius carried by the realized geometry. -/
lemma constraintRadius_pos (S : StochasticSteepestDescentGeometryContext Ω V) :
    0 < S.constraintRadius := by
  simpa [StochasticSteepestDescentGeometryContext.constraintRadius] using
    (one_div_pos.mpr S.lambda_pos)

/-- The generic primal feasibility radius is nonnegative. -/
lemma constraintRadius_nonneg (S : StochasticSteepestDescentGeometryContext Ω V) :
    0 ≤ S.constraintRadius :=
  S.constraintRadius_pos.le

/-- The realized radius specializes to the decoupled weight-decay value `1 / λ`. -/
lemma lambda_mul_constraintRadius_eq_one (S : StochasticSteepestDescentGeometryContext Ω V) :
    S.lambda * S.constraintRadius = 1 := by
  simp [StochasticSteepestDescentGeometryContext.constraintRadius]
  field_simp [S.lambda_pos.ne']

/-- The realized stochastic-gradient sample `g(W_t(ω); ζ_{t,i}(ω))`. -/
def stochasticGradientSample
    (S : StochasticSteepestDescentGeometryContext Ω V)
    (t : ℕ) (i : Fin S.batchSize) (ω : Ω) : StrongDual ℝ V :=
  S.stochasticGradientOracle.g (S.W t ω) (S.drawProcess.draw t i ω)
...
/-- The shared primal feasibility ball carried by the realized geometry. -/
...
def suboptimality (S : StochasticStarConvexGeometryContext Ω V) (t : ℕ) (ω : Ω) : ℝ :=
  S.f (S.W t ω) - S.f S.WStar

/-- The expected suboptimality gap `E[f(W_t) - f(W_*)]`. -/
def expectedSuboptimality (S : StochasticStarConvexGeometryContext Ω V) (t : ℕ) : ℝ :=
  ∫ ω, S.suboptimality t ω ∂S.μ

/-- The expected initial suboptimality `E[f(W_0) - f(W_*)]`. -/
def initialExpectedSuboptimality (S : StochasticStarConvexGeometryContext Ω V) : ℝ :=
...
/-- The stochastic suboptimality gap along a realized sample path. -/
def suboptimality (S : StochasticFrankWolfeGeometryContext Ω V) (t : ℕ) (ω : Ω) : ℝ :=
  S.f (S.W t ω) - S.f S.WStar

/-- The expected Frank-Wolfe suboptimality `E[f(W_t) - f(W_*)]`. -/
def expectedSuboptimality (S : StochasticFrankWolfeGeometryContext Ω V) (t : ℕ) : ℝ :=
  ∫ ω, S.suboptimality t ω ∂S.μ

/-- The expected initial Frank-Wolfe suboptimality `E[f(W_0) - f(W_*)]`. -/
def initialExpectedSuboptimality (S : StochasticFrankWolfeGeometryContext Ω V) : ℝ :=
  S.expectedSuboptimality 0
