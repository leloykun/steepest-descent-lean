...
/--
Assumption 1 in the realized-minibatch model.

The per-sample stochastic gradient is conditionally unbiased given the current
iterate `W_t`. The same assumption package also records the fresh-draw
independence structure at the level of the underlying draw process rather than
as an unconditional independence claim on the realized adaptive gradients
themselves.
-/
...
structure StochasticSteepestDescentGeometryContext
  ...
  grad_stronglyMeasurable :
    ∀ t, StronglyMeasurable (fun ω => fGrad (W t ω))
  WStar : V
  WStar_optimality : ∀ W, f WStar ≤ f W
  ...
  minibatchGradient_measurable :
    ∀ t, Measurable (minibatchGradient t)
  grad_prefixStronglyMeasurable :
    ∀ t (i : Fin batchSize) s, s ≤ t →
      StronglyMeasurable[samplePrefixSigma batchSize_pos W stochasticGradientSample t i]
        (fun ω => fGrad (W s ω))

  -- Momentum / search dynamics driven by the realized minibatch gradient.
  momentum : ℕ → Ω → StrongDual ℝ V
  momentum_measurable :
    ∀ t, Measurable (momentum t)
  momentum_zero_integrable :
    Integrable (momentum 0) drawProcess.μ
  momentum_succ :
    ∀ t ω,
      momentum (t + 1) ω =
        beta • momentum t ω + (1 - beta) • minibatchGradient (t + 1) ω
  C : ℕ → Ω → StrongDual ℝ V
  C_measurable :
    ∀ t, Measurable (C t)
  C_spec :
    ∀ t ω,
      C t ω = beta • momentum t ω + (1 - beta) • minibatchGradient t ω
  aStar : ℕ → Ω → V
  aStar_measurable :
    ∀ t, Measurable (aStar t)
  aStar_spec :
    ∀ t ω, IsLMO (C t ω) (aStar t ω)
  update_eq :
    ∀ t ω,
      W (t + 1) ω = ((1 - lambda * eta) • W t ω) + eta • aStar t ω
...
lemma sample_condexp_eq_grad
  ...
  intro t i
  simpa [iterateSigma, grad] using S.assumption1_sampling.estimator_condexp_eq_grad t i
...
lemma sample_condexp_eq_grad_of_prefix
    ...
    ∀ t (i : Fin S.batchSize),
      S.μ[S.stochasticGradientSample t i |
        ...
        =ᵐ[S.μ] S.grad t := by
  intro t i
  simpa [grad] using S.assumption1_sampling.estimator_condexp_eq_grad_of_prefix t i
...
lemma ξ_condexp_eq_zero_of_prefix
    ...
    ∀ t (i : Fin S.batchSize),
      S.μ[S.ξ t i |
        ...
        =ᵐ[S.μ] 0 := by
  intro t i
  let tuple := flatSamplePrefixTuple S.batchSize_pos S.stochasticGradientSample t i
  ...
  have hPrefixLe :
    ...
    simpa [samplePrefixSigma, tuple] using
      ...
      =ᵐ[S.μ]
        S.μ[S.grad t | samplePrefixSigma S.batchSize_pos S.W S.stochasticGradientSample t i]
          - S.μ[S.stochasticGradientSample t i |
            ...
            simpa [ξ, Pi.sub_apply] using hSub
    _ =ᵐ[S.μ]
        S.grad t
          - S.μ[S.stochasticGradientSample t i |
              samplePrefixSigma S.batchSize_pos S.W S.stochasticGradientSample t i] := by
...
lemma second_moment_integrable
  ...
  intro t i
  simpa [ξ, grad] using S.assumption2_secondMoment.noise_sq_integrable t i
...
lemma sample_cond_secondMoment_le
    ...
    ∀ t i,
      S.μ[fun ω => ‖S.ξ t i ω‖ ^ 2 | S.iterateSigma t] ≤ᵐ[S.μ]
        fun _ => S.sigma ^ 2 := by
  intro t i
  simpa [iterateSigma, ξ, grad, sigma] using
...
lemma second_moment_bound
  ...
  · have hCondInt :
    ...
    have hIterateSigma :
      ...
      simpa [iterateSigma] using (S.W_measurable t).comap_le
    calc
      ∫ ω, ‖S.ξ t i ω‖ ^ 2 ∂S.μ
...
lemma sample_mean_zero
  ...
  calc
    ∫ ω, S.ξ t i ω ∂S.μ
      = ∫ ω, S.grad t ω ∂S.μ - ∫ ω, S.stochasticGradientSample t i ω ∂S.μ := by
          simp [ξ, MeasureTheory.integral_sub hGradInt hSampleInt]
...
lemma sample_norm_le_noiseRadius_ae
  ...
  intro t i
  simpa [ξ, grad, noiseRadius] using S.oracle_sample_norm_le_noiseRadius_ae t i
